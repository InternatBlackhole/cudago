...
func sort(d_array *cuda.DeviceMemory, arrLen uint32, elemSize uint64, numThreads uint32) {
	...
	fmt.Printf(reportFormat, "Sort_GoStartKernelCall", fmt.Sprintf("%d", arrLen), float64(took.Nanoseconds())/1000)
	for k := 4 * int32(blockSize.X); k <= int32(arrLen); k <<= 1 { // k = 4 * blockSize ... tableLength
		for j := k / 2; j >= 2*int32(blockSize.X); j >>= 1 { //   j = k/2 ... 2 * blockSize.x
			...
			took = time.Since(startT)
			fmt.Printf(reportFormat, "Sort_GoMiddleKernelCall", fmt.Sprintf("%d", arrLen), float64(took.Nanoseconds())/1000)
		}
		startT = time.Now()
		sorting.BitonicSortFinishEx(gridSize, blockSize, bytesLocalMemory, nil, d_array.Ptr, int32(arrLen), k) //   j = 2 * blockSize.x ... 1
		took = time.Since(startT)
		fmt.Printf(reportFormat, "Sort_GoFinishKernelCall", fmt.Sprintf("%d", arrLen), float64(took.Nanoseconds())/1000)
	}
	...
	fmt.Printf(reportFormat, "Sort_KernelDur", fmt.Sprintf("%d", arrLen), elapsedTime)
